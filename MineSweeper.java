...
public class MineSweeper {
    ...
    public static final int ROWS = 15; //row
    public static final int COLS = 15; //col

    public static final double LEFT = 10; //left
    public static final double TOP = 10; //top
    ...
    public void setupGUI(){
        ...
        UI.addButton("aiHelper", this::aiHelper);
    ...
    public void tryExpose(int row, int col){
        ...
        if(square.isExposed() || square.isMarked()) return; //exposed/marked return _
        if(square.hasMine()){ // has mine lose
            drawLose();
            return;
        } else {
            exposeSquareAt(row,col); //expose square
        }
        if (hasWon()){
            drawWin(); //state win
        }
    ...
    public void exposeSquareAt(int row, int col){
        /*# YOUR CODE HERE */
        if (row<0 || row >= squares.length || col < 0 || col >= squares[row].length) {return;}
        ...
        if (squares[row][col].getAdjacentMines() ==0) {
            exposeSquareAt(row - 1, col); //up
            exposeSquareAt(row + 1, col); //down
    ...
    public int[][] readSquare(Square[][] s){
        int[][] play = new int[15][15];
        for (int row=0; row<ROWS; row++){
            for (int col=0; col<COLS; col++){
                if(s[row][col].isExposed()){
                    play[row][col] = s[row][col].getAdjacentMines();
                } else{
                    play[row][col] = -1;
                }
            }
    ...
    public void aiHelper(){
        UI.println("Sorry Boss your on your own innit");
    }
    // completed methods
